...
public: // bar bar input nile const ta change kora lagte pare may be
    ...
    void solve() {
        LRarray();
        D.resize(n + 3, 0); ll  sum = 0, cntr = 0;
        for (int i = 1; i < n; i++) {
            ll b = n - 1 - SA[i];
            uniq += (b - LCP[i]);
            if (LCP[i] == 0) {
            ...
            sum += (a * 1LL * a * (LCP[i] - mx)); cntr += (LCP[i] - mx);
            //cout << (a * 1LL * a) <<" " << (LCP[i] - mx) << endl;
        }
...
int main() {
    ...
    while(test--) {
        ...
        T[n++] = '$';  
        uniq = 0;                      // add terminating symbol
        SuffixArray S; 
        S.init(T, n);  
        for (int i = 0; i < n; i++) {
            //cout << S.LCP[i] <<" " << S.SA[i] << endl;
        }  
        S.solve();
    }
